...
class piano_dei_conti extends P4A_Mask
{
	function &piano_dei_conti ()
	{
		...
		// data sources
		$this->build("p4a_db_source", "ds_piano");
		$this->ds_piano->setTable($p4a->e3g_prefix."anagrafiche");
		$this->ds_piano->setSelect("idanag, mastro, conto, codice, descrizione, segnocontabile");		
		$this->ds_piano->setWhere("tipocfa ='S' AND mastro<>''");
		$this->ds_piano->setPk("idanag");

		$this->ds_piano->addOrder("mastro");
		$this->ds_piano->addOrder("conto");
		$this->ds_piano->addOrder("codice");
		$this->ds_piano->load();

		$this->setSource($this->ds_piano);
		$this->ds_piano->firstRow();
		...
		$table->setWidth(730);
		$table->setSource($this->ds_piano);
		$table->setVisibleCols(array("mastro", "conto","codice", "descrizione", "segnocontabile"));
